...
### "Wifi is broken / repo download failed" fallback

If the public repo at <https://github.com/Lucasmind/nodeava-workshop-public>
is unreachable for any reason (conference network, blocked port, GitHub
hiccup), the **entire USB contents are self-sufficient**. Just:

1. **Copy the entire USB folder to your laptop** (e.g., to `~/Desktop/nodeava-workshop/`
   on Mac/Linux, or `C:\nodeava-workshop\` on Windows). 23 GB transfer.
2. Run `./setup.sh` (or `setup.bat` on Windows) from the **copied folder** —
   not the USB itself. Reading from a local disk is significantly faster than
   reading from USB across the `docker load` step.
3. The scripts auto-decompress the `.tar.gz` Docker images. No manual `gunzip`
   step needed.

Once setup completes, you can unplug the USB — your laptop has everything.
