...
# Dual-mode: USB bundle present → load from tarballs (fast, offline);
# missing → pull from registries (clone-of-repo path, needs network).
# A `git clone` of the public mirror has no docker-images/ dir at all;
# a USB stick has all the .tar.gz files pre-bundled.
if [[ ! -d "$IMG_DIR" ]] || [[ -z "$(find "$IMG_DIR" -maxdepth 1 -name '*.tar.gz' -o -name '*.tar.zst' -o -name '*.tar' 2>/dev/null | head -1)" ]]; then
  warn "No image tarballs at $IMG_DIR — clone-mode: pulling from registries"
  say "  (cloned the repo? these images would be on the USB. Pulling now.)"

  # Mapping mirrors the bundle on the USB (see VERSION on a packed kit).
  # Skip kokoro-fastapi-gpu on Blackwell — same logic as the USB path below.
  count=0; skipped=0
  declare -a PULL_LIST=(
    "ghcr.io/lucasmind/nodeava-workshop/whisper-stt:known-good   nodeava-workshop-stt:latest"
    "ghcr.io/lucasmind/nodeava-workshop/frontend:latest          nodeava-workshop-frontend:latest"
    "ghcr.io/remsky/kokoro-fastapi-cpu:v0.2.4                    -"
    "searxng/searxng:latest                                       -"
  )
  if [[ "$SETUP_GPU_BLACKWELL" != "1" ]]; then
    PULL_LIST+=("ghcr.io/remsky/kokoro-fastapi-gpu:v0.2.4 -")
  else
    warn "skipped kokoro-fastapi-gpu pull — Blackwell GPU; install.sh step 8 will pin CPU TTS"
    skipped=1
  fi

  for line in "${PULL_LIST[@]}"; do
    # shellcheck disable=SC2086
    set -- $line
    image="$1"; retag="${2:--}"
    say "  pulling $image…"
    if docker pull "$image" 2>&1 | tail -1 | sed 's/^/    /'; then
      ok "pulled $image"
      count=$((count+1))
      if [[ "$retag" != "-" ]]; then
        docker tag "$image" "$retag" && ok "  retagged $image → $retag"
      fi
    else
      warn "pull failed for $image"
    fi
  done
  ok "$count image(s) pulled, $skipped skipped"

  # Skip the rest of step 4 (USB-tarball branch + retag block)
  # — clone-mode already pulled + retagged.
  :
else
  # ── USB-bundle path (original behavior) ──────────────────────────────────
...
if [[ -d "$OL_SRC" ]] && [[ -n "$(find "$OL_SRC/blobs" -type f 2>/dev/null | head -1)" ]]; then
  # ── USB-bundle path ──────────────────────────────────────────────────────
  if [[ -n "$USE_SUDO" ]]; then
  ...
  # ── Clone-mode: no bundled blobs, ollama pull from upstream ──────────────
  warn "No bundled Ollama models — clone-mode: pulling from ollama.com"
  for model in qwen3:4b-instruct smollm2:360m; do
    say "  pulling $model (one-time per machine; cached after)…"
    if ollama pull "$model" 2>&1 | grep -E 'success|pulling' | tail -2; then
      ok "pulled $model"
    else
      warn "pull failed for $model — install.sh step 6 will retry"
    fi
  done
fi
...
# Dual-mode: USB path stages `source/nodeava-workshop/` into `~/nodeava-workshop`.
# Clone-mode: USB_ROOT IS the project root (you already git-cloned it), so we
# either use it in place or copy to ~/nodeava-workshop for consistency.
SRC_DST="$HOME/nodeava-workshop"
if [[ -d "$USB_ROOT/source/nodeava-workshop" ]]; then
  SRC_FROM="$USB_ROOT/source/nodeava-workshop"
  SRC_MODE="usb"
elif [[ -f "$USB_ROOT/install.sh" ]] || [[ -f "$USB_ROOT/scripts/install.sh" ]]; then
  # Looks like a cloned repo (no source/ subdir, install.sh present at root)
  SRC_FROM="$USB_ROOT"
  SRC_MODE="clone"
  say "Detected cloned repo at $USB_ROOT — will copy in place to $SRC_DST for a consistent layout."
  say "(if you'd rather run from the clone in-place, ctrl-C and run install.sh directly from $USB_ROOT)"
else
  fail "Could not find source tree — neither $USB_ROOT/source/nodeava-workshop nor a cloned repo at $USB_ROOT"
fi

if [[ "$SRC_FROM" -ef "$SRC_DST" ]]; then
  ok "running in-place from $SRC_DST — skipping copy"
  SRC_FROM=""
elif [[ -d "$SRC_DST" ]]; then
...
if [[ -n "$SRC_FROM" ]]; then
  if [[ "$SRC_MODE" = "clone" ]]; then
    # Exclude .git, dist, node_modules, .venv to keep $HOME copy lean
    mkdir -p "$SRC_DST"
    rsync -aL \
      --exclude='.git/' --exclude='node_modules/' --exclude='dist/' \
      --exclude='.vite/' --exclude='**/__pycache__/' --exclude='**/.venv/' \
      --exclude='usb-staging/' --exclude='.claude/' \
      "$SRC_FROM/" "$SRC_DST/"
    ok "source copied from clone → $SRC_DST"
  else
    cp -R "$SRC_FROM" "$SRC_DST"
    ok "source copied from USB → $SRC_DST"
  fi
fi

# Whisper models go into the project's models/ dir (mounted into STT container).
# USB ships these; clone-mode lets the STT container download base.en at first
# run (~150 MB from huggingface).
mkdir -p "$SRC_DST/models"
if [[ -d "$USB_ROOT/whisper-models" ]]; then
  for w in "$USB_ROOT"/whisper-models/*.bin; do
    [[ -f "$w" ]] || continue
    cp -n "$w" "$SRC_DST/models/" && ok "whisper: $(basename "$w")"
  done
else
  say "No bundled whisper models — STT container will download on first run"
fi
