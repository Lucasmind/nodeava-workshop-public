...
SCRIPT_DIR="$REPO/tools/usb-setup"
...
if [[ $SKIP_DOCKER -eq 0 ]]; then
  ...
  # If running images aren't tagged exactly like above, fall back to project-prefixed names
  for img in "${IMAGES[@]}"; do
    ...
    safe="${img//\//-}"
    safe="${safe//:/-}"
    out="$STAGE/docker-images/$ARCH/${safe}.tar"
    say "  saving $img → ${safe}.tar"
    docker save -o "$out" "$img"
    ok "$(du -h "$out" | cut -f1) ${safe}.tar"
  done
else
